...
import { Download, Copy, Plus, AlertCircle, ChevronLeft, ChevronRight, Search } from 'lucide-react';
...
interface Shift {
  ...
  userName: string;
  userAvatar?: string;
  status: 'empty' | 'scheduled' | 'confirmed' | 'on_duty';
}
...
interface ScheduleTabProps {
  ...
  setScheduleTeamMode: (val: 'team' | 'my') => void;
  scheduleSearchQuery: string;
  setScheduleSearchQuery: (val: string) => void;
  scheduleRoleFilter: string;
  setScheduleRoleFilter: (val: string) => void;
  setNewShiftDay: (val: 'T2' | 'T3' | 'T4' | 'T5' | 'T6' | 'T7' | 'CN') => void;
  setNewShiftType: (val: 'morning' | 'afternoon' | 'evening') => void;
  setNewShiftStaffName: (val: string) => void;
  setNewShiftStatus: (val: 'empty' | 'scheduled' | 'confirmed' | 'on_duty') => void;
  setIsCreateShiftModalOpen: (val: boolean) => void;
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-outline-variant text-[11px] font-bold text-slate-500 uppercase tracking-widest text-center font-sans">
                ...
                <th className="py-4 px-2 text-ba-1a-1a font-sans">CN (18/10)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#ededf9]">
              ...
              <tr>
                <td className="p-4 border-r border-[#ececf7] align-middle bg-slate-50/30">
                  <div className="space-y-0.5">
                    <p className="font-bold text-xs text-slate-900 font-sans">Ca Sáng</p>
              ...
              {/* Row 2: Ca Chiều */}
              <tr>
                <td className="p-4 border-r border-[#ececf7] align-middle bg-slate-50/30 font-sans">
                  <div className="space-y-0.5">
                            ...
                            return (
                              <div key={s.id} className={`border rounded-xl p-2.5 text-xs shadow-sm bg-white space-y-2.5 ${
                                ...
                                <div className="flex items-center gap-1.5 shrink-0 select-none">
                                  ...
                                  <div className="min-w-0">
                                    <p className="font-extrabold text-[#191b23] truncate">{s.userName}</p>
                                    <p className="text-[8px] text-[#434655] font-semibold font-sans">IT Helpdesk</p>
                                  </div>
                                </div>
