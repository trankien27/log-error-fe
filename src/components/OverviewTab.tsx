...
interface OverviewTabProps {
  ...
  totalBooths: number;
  tasks: Task[];
  processedRecentActivities: Activity[];
...
export default function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#004ac6] to-[#2563eb] text-white p-6 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
        <div>
        ...
        <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">
          ...
          <span className="text-xs font-mono font-bold select-none text-white/90">2026-05-25 (GMT+7)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 shadow-sm">
        ...
        <div
          ...
          className="bg-white border border-outline-variant rounded-xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow cursor-pointer"
        >
        ...
        <div
          onClick={() => setActiveTab('tasks')}
          className="bg-red-50 border border-red-200 rounded-xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow cursor-pointer"
        >
          <div className="flex justify-between items-start">
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="text-[10px] text-[#434655] mt-1">Tài khoản quản trị</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Chart 1 */}
        <div className="lg:col-span-2 bg-white border border-outline-variant rounded-xl p-6 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-sm font-bold text-[#191b23]">Tần suất Log lỗi tuần qua</h3>
            <span className="text-xs text-primary hover:underline cursor-pointer" onClick={() => setActiveTab('error_logs')}>Chi tiết</span>
          </div>
      ...
      <div className="bg-white border border-outline-variant rounded-xl overflow-hidden shadow-sm">
        <div className="px-5 py-4 border-b border-outline-variant flex justify-between items-center bg-gray-50">
          ...
          <button onClick={() => triggerToast('Tất cả lịch sử tác vụ đã được đồng bộ chuẩn chỉnh.')} className="text-xs font-bold text-[#004ac6] hover:underline cursor-pointer">
            Xem tất cả &rarr;
          </button>
        </div>
        <div className="divide-y divide-[#f1f5f9]">
          {processedRecentActivities.map(item => (
